...
function buildHomepageTrackingScript() {
  return `<script>${UTMIFY_PIXEL_LOADER}(function(){var checkoutLocked=false;var trackingKeys=/^(utm_|fbclid$|gclid$|gbraid$|wbraid$|ttclid$|tbclid$|xcod$|sck$|src$)/i;function persistTracking(){try{var stored=new URLSearchParams(localStorage.getItem("tracking_utm")||"");var current=new URLSearchParams(window.location.search);current.forEach(function(value,key){if(value&&trackingKeys.test(key))stored.set(key,value)});var qs=stored.toString();if(qs)localStorage.setItem("tracking_utm",qs);return qs}catch(err){return window.location.search.replace(/^\\?/,"")}}function ready(){return typeof window.fbq==="function"}function waitForPixel(done){if(ready()){done(true);return}var started=Date.now();var timer=setInterval(function(){if(ready()){clearInterval(timer);done(true)}else if(Date.now()-started>=5500){clearInterval(timer);done(false)}},50)}var proxy=document.createElement("a");proxy.id="utmify-checkout-proxy";proxy.hidden=true;proxy.tabIndex=-1;document.body.appendChild(proxy);persistTracking();document.addEventListener("click",function(e){var target=e.target;var btn=target&&target.closest?target.closest("button"):null;if(!btn)return;var label=(btn.textContent||"").replace(/\\s+/g," ").trim();if(label!=="Comprar agora"&&label!=="Adicionar ao carrinho")return;e.preventDefault();e.stopImmediatePropagation();if(checkoutLocked)return;checkoutLocked=true;window.__showProductLoader&&window.__showProductLoader();var item={id:"6549324",title:"Jogo De Panelas Indução Antiaderente Cerâmica 10 Peças PPG PFOA Free Baunilha",price:"61,93",image:"https://i.postimg.cc/Gtj1SkJR/D-NQ-NP-2X-754218-MLA98733384331-112025-F.webp"};try{localStorage.setItem("checkout_product",JSON.stringify(item))}catch(err){}var qs=persistTracking();proxy.href="/endereco?checkout=1"+(qs?"&"+qs:"");window.__utmify_ic_status={waiting:true,at:Date.now(),product:item.id};waitForPixel(function(ok){window.__utmify_ic_status={ready:ok,replayed:ok,at:Date.now(),product:item.id};if(ok){setTimeout(function(){window.location.assign(proxy.href)},3000);proxy.click()}else{window.location.assign(proxy.href)}})},true);})();</script>`;
}
