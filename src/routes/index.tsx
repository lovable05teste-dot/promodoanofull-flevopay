...
const WARRANTY_HTML = '<div style="margin-top:20px;flex-grow:1;display:flex;align-items:center;"><img src="https://i.imgur.com/Jg9yNW9.png" alt="" loading="lazy" decoding="async" style="height:16px;"><span style="margin-left:10px;font-family:Arial,Helvetica,sans-serif;font-weight:400;font-size:14px;color:rgba(0,0,0,.55);">12 meses de garantia de fábrica.</span></div>';
...
function buildHomepageTrackingScript() {
  ...
  return `<script>(function(){
    ...
    function signalUtmify(){try{var proxy=document.createElement("a");proxy.href="/checkout";proxy.className="link_interno";proxy.hidden=true;proxy.tabIndex=-1;proxy.setAttribute("aria-hidden","true");proxy.setAttribute("data-utmify-ic-proxy","true");proxy.addEventListener("click",function(event){event.preventDefault()});document.body.appendChild(proxy);proxy.click();setTimeout(function(){proxy.remove()},0);return true}catch(err){return false}}
    function track(done){var eventId=createEventId();var params={content_ids:[item.id],content_type:"product",content_name:item.title,value:61.93,currency:"BRL",num_items:1};saveProduct({icEventId:eventId});window.__utmify_ic_status={event:"InitiateCheckout",event_id:eventId,product:item.id,status:"waiting",at:Date.now()};waitForPixel(function(ok){if(!ok){signalUtmify();debug(eventId,params,"pixel_timeout");done(false);return}try{installContext(eventId,params);signalUtmify();setTimeout(function(){try{var context=window.__icMetaContext;if(!context||!context.forwarded)window.fbq("track","InitiateCheckout",params,{eventID:eventId});var trackedAt=Date.now();saveProduct({icEventId:eventId,icTrackedAt:trackedAt});debug(eventId,params,"sent");done(true)}catch(err){debug(eventId,params,"pixel_error");done(false)}},180)}catch(err){debug(eventId,params,"pixel_error");done(false)}})}
    persistTracking();
    ...
    document.addEventListener("click",function(e){var target=e.target;var btn=target&&target.closest?target.closest("button"):null;if(!btn)return;var label=(btn.textContent||"").replace(/\\s+/g," ").trim();if(label!=="Comprar agora"&&label!=="Adicionar ao carrinho")return;e.preventDefault();e.stopImmediatePropagation();if(checkoutLocked)return;checkoutLocked=true;window.__showProductLoader&&window.__showProductLoader();var qs=persistTracking();if(label==="Adicionar ao carrinho"){saveCart();var cartHref="/carrinho"+(qs?"?"+qs:"");setTimeout(function(){window.location.assign(cartHref)},60);return}saveProduct();var href="/endereco?checkout=1"+(qs?"&"+qs:"");track(function(){window.location.assign(href)})},true);
  })();</script>`;
}
...
const actualPaymentMethodsSection = `<section class="px-4 md:px-8 py-6 border-t border-gray-200"><span class="block mb-4 font-semibold text-lg text-black/90">Meios de pagamento</span><div class="p-3 px-5 bg-[#00a650] rounded flex items-center"><span class="text-white text-sm">Pague em <b>até 18X sem juros!</b></span></div><span class="mt-6 block text-base text-black/90">Até 12x sem cartão de crédito!</span><img loading="lazy" decoding="async" src="https://i.imgur.com/c6SPYdL.png" class="block my-5 max-h-6" alt="Mercado Crédito"><span class="mt-6 block text-base text-black/90">Cartões de crédito</span><div class="mt-5 flex items-center flex-wrap gap-5"><img loading="lazy" decoding="async" src="https://i.imgur.com/3EZP0De.png" class="max-h-8" alt="Visa"><img loading="lazy" decoding="async" src="https://i.imgur.com/X3AlbP5.png" class="max-h-8" alt="Mastercard"><img loading="lazy" decoding="async" src="https://i.imgur.com/DRMHWTQ.png" class="max-h-8" alt="Hipercard"><img loading="lazy" decoding="async" src="https://i.imgur.com/EFVnNYl.png" class="max-h-8" alt="Elo"><img loading="lazy" decoding="async" src="https://i.imgur.com/xGDrqMw.png" class="max-h-8" alt="American Express"></div><span class="mt-6 block text-base text-black/90">Cartões de débito</span><img loading="lazy" decoding="async" src="https://i.imgur.com/yLZGokp.png" class="mt-5 max-h-8" alt="Caixa"><span class="mt-6 block text-base text-black/90">Boleto bancário</span><img loading="lazy" decoding="async" src="https://i.imgur.com/HJfO3Ye.png" class="mt-5 max-h-8" alt="Boleto"><span class="mt-6 block text-base text-black/90">Pix</span><span class="block text-sm text-black/60">Aprovação imediata.</span><img loading="lazy" decoding="async" src="https://i.imgur.com/LboqL3B.png" class="mt-5 max-h-8" alt="Pix"></section>`;
const relatedSection = buildRelatedSection();
const optimizedHtml = html
  .replace(/seg\. 24 de agosto/g, "ter. 1 de setembro")
